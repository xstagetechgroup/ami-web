        ...
        <div className="w-full">
            ...
            <Container>
                <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-5">
                    ...
                    {/* formulário — temporariamente desativado */}
                    <div className="py-20 w-full flex flex-col gap-5 justify-center items-center text-center">
                        <div className="w-16 h-16 rounded-full bg-pink-50 flex items-center justify-center">
                            <FaEnvelope className="size-7 text-primaryColor" />
                        </div>
                        <h2 className="text-2xl font-bold text-gray-800">Formulário em breve</h2>
                        <p className="text-gray-500 max-w-sm">
                            O formulário de contacto está temporariamente indisponível.
                            Pode contactar-nos diretamente por email ou telefone.
                        </p>
                        <Link
                            href="mailto:geral@ami.org.ao"
                            className="mt-2 px-6 py-3 bg-primaryColor text-white rounded-lg font-semibold hover:opacity-90 transition-opacity"
                        >
                            geral@ami.org.ao
                        </Link>
                    </div>

                    {/* FORMULÁRIO COMENTADO — descomentar quando RESEND_API_KEY estiver configurado */}
                    {/*
                    <div className="py-20 w-full flex flex-col gap-5 justify-start items-start">
                        ...
                        <form className="w-full" onSubmit={handleSubmit}>
                            ...
                            <Input placeholder={c.form.namePlaceholder} type="text" name="name"
                                value={form.name} onChange={handleChange} className="rounded-md w-full mt-1 p-5" />

                            <Label className="font-normal mt-5">{c.form.email}</Label>
                            <Input placeholder={c.form.emailPlaceholder} type="email" name="email"
                                value={form.email} onChange={handleChange} className="rounded-md w-full mt-1 p-5" />

                            <Label className="font-normal mt-5">{c.form.subject}</Label>
                            <Input placeholder={c.form.subjectPlaceholder} type="text" name="subject"
                                value={form.subject} onChange={handleChange} className="rounded-md w-full mt-1 p-5" />

                            <Label className="font-normal mt-5">{c.form.message}</Label>
                            <Textarea placeholder={c.form.messagePlaceholder} rows={5} name="message"
                                value={form.message} onChange={handleChange} className="rounded-md w-full mt-1" />

                            <Button type="submit" disabled={loading}
                                className="rounded-lg bg-primaryColor w-full mt-5 py-7 text-lg">
